...
const umbrellaResource = '/aether-roc-umbrella';
const grafanaResource = 'grafana';
...
export class K8sClientService {
    ...
    getGrafanaSecret(): Observable<Grafana> {
        const fullUrl: string = this.kubernetesApiUrl + namespacesResource + '/' + this.namespace +
            secretResource + umbrellaResource + '-' + grafanaResource;
        return this.http.get<Grafana>(fullUrl).pipe(
            pluck('data')
        );
    }
}
